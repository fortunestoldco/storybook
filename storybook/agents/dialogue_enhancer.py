...
class DialogueEnhancer(BaseAgent):
    ...
    def enhance_dialogue(
        ...
        dialogue_analysis = self._analyze_dialogue_quality(
            dialogue_sections, characters, target_audience
        )

        # Determine improvement strategies
        improvement_strategies = self._determine_improvement_strategies(
            dialogue_analysis, characters, target_audience, research_insights
        )
            ...
            if in_section and not has_dialogue:
                ...
                for j in range(1, 3):
                    if i + j < len(paragraphs) and not any(
                        m in paragraphs[i + j] for m in dialogue_markers
                    ):
                        no_dialogue_streak += 1
    ...
    def _analyze_dialogue_quality(
        ...
        character_voice_info = (
            ...
            if character_voices
            else "No character voice information available."
        )
    ...
    def _determine_improvement_strategies(
        self,
