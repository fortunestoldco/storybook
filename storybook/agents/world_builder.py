...
class WorldBuilder:
    ...
    def _extract_settings(self, content: str, target_audience: Optional[Dict[str, Any]] = None,
    ...
    def _develop_setting(self, manuscript_id: str, setting: Dict[str, Any], content: str,
        ...
        sections = ["Physical Description", "History & Background", "Cultural Significance", 
                   "Atmosphere & Mood", "Symbolic Meaning", "Points of Interest", 
                   "Potential Development", "Target Audience Appeal"]
        ...
        for i in range(len(sections)):
            ...
            end_idx = profile_text.find(end_marker, start_idx) if end_marker else len(profile_text)
    ...
    def _analyze_world_consistency(self, settings: List[Dict[str, Any]], content: str,
    ...
    def _enhance_setting_descriptions(self, content: str, settings: List[Dict[str, Any]],
        ...
        priority_settings = sorted(
            settings, 
            key=lambda x: 0 if x.get("importance", "").lower() == "primary" else 
                         (1 if x.get("importance", "").lower() == "secondary" else 2)
        )
        ...
        for setting in settings_to_enhance:
            ...
            pattern = re.compile(r'(\.\s+|^)([^.!?]*\b' + re.escape(setting_name) + r'\b[^.!?]*[.!?])')
            ...
            profile_parts = []
            if "physical_description" in setting:
                profile_parts.append(f"Physical Description: {setting['physical_description']}")
            if "atmosphere_&_mood" in setting:
                profile_parts.append(f"Atmosphere & Mood: {setting['atmosphere_&_mood']}")
            ...
            updated_content = updated_content.replace(original_sentence, enhanced_description, 1)
