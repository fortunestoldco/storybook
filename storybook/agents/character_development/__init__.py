...
class CharacterPersonality(BaseModel):
    ...
    traits: Dict[str, float] = Field(description="Personality traits with 0-1 scale ratings")
    cognitive_style: str = Field(description="How the character thinks and processes information")
    emotional_patterns: Dict[str, str] = Field(description="Emotional response patterns")
    ...
    stress_responses: Dict[str, str] = Field(description="How the character responds to different stressors")
    growth_potential: Dict[str, str] = Field(description="Areas where character could grow or change")

class CharacterArc(BaseModel):
    """Character arc model."""
    starting_state: Dict[str, Any] = Field(description="Character's state at the beginning")
    key_moments: List[Dict[str, str]] = Field(description="Key transformational moments")
    ...
    external_manifestation: str = Field(description="How internal changes manifest externally")
...
class DialoguePattern(BaseModel):
    ...
    avoidances: List[str] = Field(description="Topics or speech patterns this character avoids")
    emotion_indicators: Dict[str, List[str]] = Field(description="How different emotions manifest in speech")
...
class CharacterResearchAgent:
    ...
    def create_character_profile(self, name: str, role: str, genre: str, themes: List[str]) -> Character:
        ...
        dialogue_patterns = self._create_dialogue_patterns(name, personality, background)
        
        # Identify and subvert tropes
        tropes, subversions = self._identify_and_subvert_tropes(role, genre, personality)
    ...
    def _create_personality_profile(self, name: str, role: str, genre: str, themes: List[str]) -> CharacterPersonality:
...
""",
        ...
        try:
            ...
            return CharacterPersonality(
                ...
                emotional_patterns={"stress": "Becomes withdrawn", "joy": "Expressive and generous"},
                motivation_drivers=["Achievement", "Connection"],
                values=[{"name": "Loyalty", "description": "Values commitment to others"}],
                shadow_traits=["Self-doubt", "Fear of abandonment"],
                stress_responses={"conflict": "Becomes defensive", "failure": "Self-critical"},
                growth_potential={"empathy": "Could develop greater understanding of others"}
            )
    
    def _create_character_background(self, name: str, role: str, personality: CharacterPersonality, genre: str) -> str:
...
""",
        ...
        for line in response.content.split("\n"):
            line = line.strip()
            if line and (":" in line or "." in line) and any(char.isdigit() for char in line[:2]):
    ...
    def _create_initial_arc(self, name: str, role: str, personality: CharacterPersonality, themes: List[str]) -> CharacterArc:
...
""",
        ...
        try:
            ...
            return CharacterArc(
                ...
                key_moments=[{"moment": "Challenge", "description": "Character faces a significant challenge"}],
    ...
    def _create_dialogue_patterns(self, name: str, personality: CharacterPersonality, background: str) -> DialoguePattern:
...
""",
        ...
        response = self.llm.invoke(prompt.format(
            name=name,
            personality=json.dumps(personality.model_dump(), indent=2),
            background=background[:1000]  # Truncate background to avoid token limits
        ))
        ...
        try:
            ...
            return DialoguePattern(
                ...
                emotion_indicators={"angry": ["Short sentences", "Increased volume"], 
                                   "happy": ["More animated speech", "Laughs frequently"]}
            )
    
    def _identify_and_subvert_tropes(self, role: str, genre: str, personality: CharacterPersonality) -> tuple[List[str], List[str]]:
...
""",
        ...
        response = self.llm.invoke(prompt.format(
            role=role,
            genre=genre,
            personality_traits=traits_text
        ))
        ...
        for line in response.content.split("\n"):
            ...
            if current_section == "tropes" and any(char.isdigit() for char in line[:2]):
            ...
            elif current_section == "subversions" and any(char.isdigit() for char in line[:2]):
                ...
                if ":" in line:
                    ...
                    if "[" in subversion and "]" in subversion:
                        subversion = subversion.split("[", 1)[1].split("]", 1)[0].strip()
...
class CharacterArcDesignerAgent:
    ...
    def refine_character_arc(self, character: Character, plot_points: List[Any], themes: List[str]) -> Dict[str, Any]:
...
""",
            input_variables=["name", "role", "background", "initial_arc", "plot_points", "themes"]
        )
        
        # Format plot points for the prompt
        plot_points_text = ""
        for i, point in enumerate(plot_points[:10]):  # Limit to first 10 to avoid token limits
        ...
        response = self.llm.invoke(prompt.format(
            name=character.name,
            role=character.role,
            background=character.background[:500] + "...",  # Truncate to avoid token limits
        ...
        for line in response.content.split("\n"):
            if "MAP ARC TO PLOT" in line or "1. MAP ARC TO PLOT" in line:
                ...
                continue
            elif "EMOTIONAL TRANSFORMATION" in line or "3. EMOTIONAL TRANSFORMATION" in line:
                current_section = "emotional_transformation"
                continue
            elif "EXTERNAL MANIFESTATION" in line or "4. EXTERNAL MANIFESTATION" in line:
        ...
        for line in sections["arc_plot_mapping"].split("\n"):
            if ":" in line and any(char.isdigit() for char in line[:2]):
                moment = {"moment": line.split(":", 1)[0].strip(), "description": line.split(":", 1)[1].strip()}
        ...
        refined_arc["external_manifestation"] = sections["external_manifestation"].strip()
        ...
        return {
            ...
            "arc_document": response.content
        }
    
    def create_transformation_scenes(self, character: Character, refined_arc: Dict[str, Any]) -> List[Dict[str, Any]]:
...
""",
        ...
        personality_summary = "Key traits: " + ", ".join([f"{trait} ({value})" for trait, value in personality_traits.items()])
        ...
        for line in response.content.split("\n"):
            ...
            if ":" in line and line.split(":")[0] in ["SCENE TITLE", "STORY POSITION", "SETTING", "SITUATION", 
                                                      "CHARACTER RESPONSE", "KEY DIALOGUE/ACTIONS", "INTERNAL TRANSFORMATION"]:
                field = line.split(":")[0]
...
class CharacterRelationshipMapperAgent:
    ...
    def map_relationships(self, characters: Dict[str, Character]) -> Dict[str, Dict[str, Any]]:
        """Map relationships between all characters."""
        if not characters:
            return {}
            
        character_summaries = {name: self._summarize_character(char) for name, char in characters.items()}
...
""",
    ...
    def _summarize_character(self, character: Character) -> str:
        ...
        if character.personality and "traits" in character.personality:
            ...
            summary += "Key traits: " + ", ".join([f"{t}" for t in traits.keys()]) + "\n"
    ...
    def update_character_relationships(self, characters: Dict[str, Character], relationship_map: Dict[str, Dict[str, Any]]) -> Dict[str, Character]:
        ...
        for char_name, relationships in relationship_map.items():
            if char_name in updated_characters:
                ...
                char.relationships = {other: desc for other, desc in relationships.items()}
...
""",
            input_variables=["name", "role", "background", "personality_traits", "current_patterns"]
        )
        ...
        current_patterns = json.dumps(character.dialogue_patterns, indent=2) if character.dialogue_patterns else "Not yet developed"
        ...
        refined_patterns = character.dialogue_patterns.copy() if character.dialogue_patterns else {}
        ...
        refined_patterns["sentence_structures"] = sections["sentence_structures"].strip()
        
        # Extract verbal quirks as a list
        verbal_quirks = []
        for line in sections["verbal_quirks"].split("\n"):
            if line.strip() and ("- " in line or "• " in line or any(char.isdigit() for char in line[:2])):
                quirk = line.split("- ", 1)[-1].split("• ", 1)[-1]
                if any(char.isdigit() for char in line[:2]):
                    quirk = line.split(".", 1)[-1].strip()
                verbal_quirks.append(quirk)
        
        refined_patterns["verbal_quirks"] = verbal_quirks if verbal_quirks else sections["verbal_quirks"].strip()
        refined_patterns["dialogue_samples"] = dialogue_samples
        refined_patterns["emotional_speech_patterns"] = sections["emotional_patterns"].strip()
    ...
    def generate_dialogue_examples(self, character: Character, situations: List[str]) -> Dict[str, str]:
...
...
""",
            input_variables=["name", "role", "background", "dialogue_patterns", "situations"]
        )
        ...
        dialogue_patterns = json.dumps(character.dialogue_patterns, indent=2) if character.dialogue_patterns else "Not yet developed"
