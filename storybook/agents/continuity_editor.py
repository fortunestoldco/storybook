...
class ContinuityEditor:
    ...
    def __init__(self):
        self.llm = get_llm(temperature=0.3, use_replicate=True)  # Lower temperature for more consistent analysis
    ...
    def _identify_continuity_issues(self, content: str) -> Dict[str, Any]:
        ...
        for i, chunk in enumerate(chunks):
            excerpts_text.append(f"EXCERPT {i+1} (approximately {i+1}/{len(chunks)} of the way through):\n{chunk}")
        ...
        try:
            ...
            issue_blocks = re.findall(r'(\d+\.\s+Issue Type:.*?)(?=\d+\.\s+Issue Type:|$)', issues_str, re.DOTALL)
    ...
    def _fix_continuity_issues(self, content: str, continuity_issues: Dict[str, Any]) -> tuple:
            ...
            chain = (
                {
                    ...
                    "problematic_text": lambda _: self._get_context(updated_content, location_snippet),
