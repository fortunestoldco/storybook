...
class QualityReviewer:
    ...
    def finalize_manuscript(self, manuscript_id: str, target_audience: Optional[Dict[str, Any]] = None,
    ...
    def _review_quality(self, content: str, target_audience: Optional[Dict[str, Any]] = None,
                    research_insights: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        aspects = ["Overall Narrative Quality", "Character Development", "World-Building", 
                  "Plot Coherence", "Pacing", "Dialogue", "Prose Quality", 
                  "Thematic Depth", "Market Readiness", "Reader Engagement"]
        ...
        assessment_match = re.search(r'Overall(?: Assessment)?:?(.*?)(?=\n\n|\Z)', review, re.IGNORECASE | re.DOTALL)
        overall_assessment = assessment_match.group(1).strip() if assessment_match else ""
    ...
    def _identify_improvement_opportunities(self, content: str, quality_review: Dict[str, Any],
        ...
        critical_match = re.search(r'Critical Issues:?(.*?)(?=\n\n\d+\.|Quick Wins:|Long-term Revisions:|\Z)', opportunities, re.IGNORECASE | re.DOTALL)
        critical_issues = critical_match.group(1).strip() if critical_match else ""
        
        # Extract quick wins
        quick_match = re.search(r'Quick Wins:?(.*?)(?=\n\n\d+\.|Critical Issues:|Long-term Revisions:|\Z)', opportunities, re.IGNORECASE | re.DOTALL)
        quick_wins = quick_match.group(1).strip() if quick_match else ""
        
        # Extract long-term revisions
        longterm_match = re.search(r'Long-term Revisions:?(.*?)(?=\n\n\d+\.|Critical Issues:|Quick Wins:|\Z)', opportunities, re.IGNORECASE | re.DOTALL)
        ...
        def extract_issues(text):
            ...
            if not issue_blocks:
                ...
                issue_blocks = re.findall(r'((?:Issue\s+)?\d+:?\s+.*?)(?=\n\n(?:Issue\s+)?\d+:?|\Z)', text, re.DOTALL)
    ...
    def _generate_final_report(self, manuscript_id: str, manuscript_title: str,
        ...
        chain = (
            {
                ...
                "overall_assessment": lambda _: quality_review.get("overall_assessment", ""),
                "critical_issues_count": lambda _: len(improvement_opportunities.get("critical_issues", [])),
                "quick_wins_count": lambda _: len(improvement_opportunities.get("quick_wins", [])),
                "longterm_revisions_count": lambda _: len(improvement_opportunities.get("longterm_revisions", [])),
                "audience_context": lambda _: audience_context,
                "research_context": lambda _: research_context
            }
            | prompt
    ...
    def _make_final_improvements(self, content: str, improvement_opportunities: Dict[str, Any],
        ...
        for issue in issues_to_address:
            ...
            if any(term in issue_lower for term in ["beginning", "start", "opening", "introduction"]):
                # Insert near beginning (after first 10% of content)
                insertion_point = len(updated_content) // 10
            elif any(term in issue_lower for term in ["end", "conclusion", "resolution", "final"]):
