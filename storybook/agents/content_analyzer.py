...
class ContentAnalyzer:
    ...
    def __init__(self):
        self.llm = get_llm(temperature=0.4, use_replicate=True)  # Lower temperature for more consistent analysis
    ...
    def analyze_progress(self, manuscript_id: str, previous_analysis: Dict[str, Any], 
        ...
        if stage == "story_arc":
            ...
            progress_analysis["structure_improvements"] = self._compare_analysis(
                previous_analysis.get("content_structure", {}),
                new_structure
            )
            ...
            progress_analysis["style_improvements"] = self._compare_analysis(
                previous_analysis.get("writing_style", {}),
                new_style
            )
            progress_analysis["readability_improvements"] = self._compare_analysis(
                previous_analysis.get("readability", {}),
                new_readability
            )
            ...
            progress_analysis["overall_improvements"] = {
                "sentiment": self._compare_analysis(
                    previous_analysis.get("sentiment", {}),
                    new_sentiment
                ),
            ...
            progress_analysis["improvement_summary"] = self._generate_improvement_summary(
                previous_analysis,
                {
    ...
    def _analyze_sentiment(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            tone_match = re.search(r'overall_tone"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
            if tone_match:
                result["overall_tone"] = tone_match.group(1).strip()
            
            emotions_match = re.search(r'dominant_emotions"?\s*:?\s*(?:\[([^\]]+)\]|"([^"]+)")', analysis_str)
            if emotions_match:
                emotions_str = emotions_match.group(1) or emotions_match.group(2)
                result["dominant_emotions"] = [e.strip().strip('"\'') for e in emotions_str.split(',')]
    ...
    def _analyze_readability(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            grade_match = re.search(r'grade_level"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
            if grade_match:
                result["grade_level"] = grade_match.group(1).strip()
            
            vocab_match = re.search(r'vocabulary_complexity"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
            if vocab_match:
                result["vocabulary_complexity"] = vocab_match.group(1).strip()
            
            readability_match = re.search(r'overall_readability"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
    ...
    def _analyze_structure(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            structure_match = re.search(r'narrative_structure"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
            if structure_match:
                result["narrative_structure"] = structure_match.group(1).strip()
            
            pacing_match = re.search(r'pacing_analysis"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
            if pacing_match:
                result["pacing_analysis"] = pacing_match.group(1).strip()
            
            balance_match = re.search(r'content_balance"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
            if balance_match:
    ...
    def _analyze_style(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            dialogue_match = re.search(r'dialogue_style"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
            if dialogue_match:
    ...
    def _analyze_genre_match(self, content: str, title: str) -> Dict[str, Any]:
        ...
        try:
            ...
            genre_match = re.search(r'primary_genre"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
            if genre_match:
                result["primary_genre"] = genre_match.group(1).strip()
            
            subgenres_match = re.search(r'subgenres"?\s*:?\s*(?:\[([^\]]+)\]|"([^"]+)")', analysis_str)
            if subgenres_match:
                subgenres_str = subgenres_match.group(1) or subgenres_match.group(2)
                result["subgenres"] = [g.strip().strip('"\'') for g in subgenres_str.split(',')]
            
            audience_match = re.search(r'genre_target_audience"?\s*:?\s*"?([^",\}]+)"?', analysis_str)
            if audience_match:
                result["genre_target_audience"] = audience_match.group(1).strip()
            
            return result
    
    def _compare_analysis(self, previous: Dict[str, Any], current: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_improvement_summary(self, initial: Dict[str, Any], final: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "improvement_metrics": {
                "sentiment_change": self._calculate_metric_change(
                    initial.get("sentiment", {}), 
                    final.get("sentiment", {})
                ),
                "readability_change": self._calculate_metric_change(
                    initial.get("readability", {}), 
                    final.get("readability", {})
                ),
                ...
                "style_change": self._calculate_metric_change(
                    initial.get("writing_style", {}), 
                    final.get("writing_style", {})
                )
            }
        }
