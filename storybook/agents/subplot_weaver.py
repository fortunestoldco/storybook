...
class SubplotWeaver:
    ...
    def __init__(self):
        self.llm = get_llm(temperature=0.8, use_replicate=True)  # Higher creativity for subplots
        self.document_store = DocumentStore()
    
    def weave_subplots(self, manuscript_id: str, characters: List[Dict[str, Any]],
    ...
    def _identify_subplots(self, content: str, characters: List[Dict[str, Any]],
        ...
        try:
            ...
            subplot_blocks = re.findall(r'(\d+\.\s+Title:.*?)(?=\d+\.\s+Title:|$)', subplots_str, re.DOTALL)
            ...
            for block in subplot_blocks:
                ...
                # Extract connection
                conn_match = re.search(r'Connection to Main Plot:\s*(.*?)(?=\n|$)', block, re.DOTALL)
                if conn_match:
    ...
    def _analyze_subplot_potential(self, content: str, characters: List[Dict[str, Any]], 
        ...
        existing_subplot_text = "\n\n".join(existing_subplot_summaries) if existing_subplot_summaries else "No existing subplots identified."
    ...
    def _develop_subplots(self, manuscript_id: str, existing_subplots: List[Dict[str, Any]],
                      subplot_potential: Dict[str, Any], characters: List[Dict[str, Any]],
                      target_audience: Optional[Dict[str, Any]] = None,
        ...
        num_new_subplots = min(2, 3 - len(existing_subplots)) if len(existing_subplots) < 3 else 0
        ...
        for subplot in developed_subplots:
            ...
            subplot_id = self.document_store.store_subplot(
                manuscript_id,
                subplot.get("title", "Untitled Subplot"),
                subplot
            )
    ...
    def _enhance_existing_subplot(self, manuscript_id: str, subplot: Dict[str, Any],
        ...
        chain = (
            {
                ...
                "subplot_connection": lambda _: subplot.get("connection_to_main_plot", ""),
                "character_info": lambda _: character_text,
                "audience_context": lambda _: audience_context
            }
            | prompt
        ...
        sections = ["Enhanced Subplot Arc", "Key Scenes", "Character Development", 
                   "Thematic Enhancement", "Integration Points", "Resolution"]
        ...
        for i in range(len(sections)):
            ...
            end_idx = enhanced_subplot.find(end_marker, start_idx) if end_marker else len(enhanced_subplot)
    ...
    def _create_new_subplots(self, manuscript_id: str, num_subplots: int,
                         existing_subplots: List[Dict[str, Any]],
        ...
        existing_subplot_titles = [subplot.get("title", "Untitled") for subplot in existing_subplots]
        existing_subplot_text = ", ".join(existing_subplot_titles) if existing_subplot_titles else "None"
        
        # Format character information
        character_info = []
        for char_name, char_data in list(character_dict.items())[:5]:  # Limit to top 5 characters
            info = f"Character: {char_name}\n"
            if "motivations" in char_data:
        ...
        chain = (
            {
                ...
                "existing_subplots": lambda _: existing_subplot_text,
                "potential_analysis": lambda _: subplot_potential.get("potential_analysis", ""),
                "character_info": lambda _: character_text,
                "audience_context": lambda _: audience_context
            }
            | prompt
        ...
        subplot_blocks = re.split(r'(?:Subplot\s+\d+:|New Subplot\s+\d+:|#\d+\s+Subplot|\d+\.\s+Subplot)', new_subplots_text)
    ...
    def _parse_new_subplot(self, subplot_text: str, manuscript_id: str) -> Optional[Dict[str, Any]]:
        """Parse a new subplot from text."""
        # Define the sections we expect to find
        sections = ["Title", "Characters Involved", "Subplot Arc", "Key Scenes", 
                   "Character Development", "Thematic Value", "Integration Points", "Resolution"]
    ...
    def _integrate_subplots(self, content: str, developed_subplots: List[Dict[str, Any]],
        ...
        priority_subplots = sorted(
            developed_subplots,
            key=lambda x: 0 if "integration_points" in x else 1
        )
        ...
        for subplot in subplots_to_integrate:
            ...
            chain = (
                {
                    ...
                    "subplot_characters": lambda _: subplot.get("characters_involved", ""),
            ...
            # Run the chain
            new_scene = chain.invoke(f"Create integration scene for {subplot.get('title', 'subplot')}")
            ...
            offset = (subplot_idx - len(subplots_to_integrate) // 2) * (len(updated_content) // 8)
