...
class TestDocumentStore:
    ...
    def mock_mongodb_store(self):
        with patch('storybook.db.document_store.MongoDBStore') as mock_store:
            ...
            mock_store_instance.get_document.return_value = {"title": "Test Title", "content": "Test Content"}
            mock_store_instance.update_document.return_value = True
            mock_store_instance.store_documents_with_embeddings.return_value = ["doc1", "doc2"]
            
            yield mock_store_instance
    
    @pytest.fixture
    def mock_text_splitter(self):
        with patch('storybook.db.document_store.RecursiveCharacterTextSplitter') as mock_splitter:
    ...
    def test_store_manuscript(self, mock_mongodb_store):
        ...
        result = store.store_manuscript("Test Title", "Test Content", {"author": "Test Author"})
        
        # Verify
        mock_mongodb_store.store_document.assert_called_once_with(
            "manuscripts", 
            {"title": "Test Title", "content": "Test Content", "metadata": {"author": "Test Author"}}
        )
    ...
    def test_get_manuscript(self, mock_mongodb_store):
        ...
        # Verify
        mock_mongodb_store.get_document.assert_called_once_with("manuscripts", "test_id")
    ...
    def test_store_research_from_web(self, mock_mongodb_store, mock_web_loader, mock_text_splitter):
        # Setup
        store = DocumentStore()
    ...
    def test_get_manuscript_relevant_parts(self, mock_mongodb_store):
        ...
        doc1 = Document(page_content="Relevant part 1", metadata={"manuscript_id": "test_id"})
        doc2 = Document(page_content="Relevant part 2", metadata={"manuscript_id": "test_id"})
        ...
        mock_mongodb_store.query_documents.return_value = [{"content": "Full content", "title": "Test"}]
