...
class ManuscriptInput(BaseModel):
    ...
    target_audience: Optional[str] = Field(None, description="Target audience (if known)")
    genre: Optional[str] = Field(None, description="Genre of the manuscript (if known)")
    author_notes: Optional[str] = Field(None, description="Additional notes from the author")
...
class ManuscriptOutput(BaseModel):
    ...
    final_report: Optional[str] = Field(None, description="Final report on the transformed manuscript")
    research_insights: Optional[Dict[str, Any]] = Field(None, description="Research insights gathered")
    analysis_results: Optional[Dict[str, Any]] = Field(None, description="Analysis results of the manuscript")
    improvement_metrics: Optional[Dict[str, Any]] = Field(None, description="Metrics on improvements made")
...
class ManuscriptUpload(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(None, description="Optional metadata about the manuscript")

class ManuscriptTransformRequest(BaseModel):
    manuscript_id: str = Field(..., description="ID of the manuscript to transform")

class TransformationStatus(BaseModel):
    job_id: str = Field(..., description="Job ID for tracking the transformation process")
    status: str = Field(..., description="Current status of the transformation")
    current_state: Optional[str] = Field(None, description="Current state in the workflow")
...
add_routes(
    app,
    novel_transformation_graph.with_types(
        input_type=ManuscriptInput,
        output_type=ManuscriptOutput
    ),
    path="/transform",
)
...
async def upload_manuscript(manuscript: ManuscriptUpload):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to upload manuscript: {str(e)}")
...
async def get_manuscript(manuscript_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve manuscript: {str(e)}")
