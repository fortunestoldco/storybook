...
def create_research_graph(config: Config):
    ...
    def conduct_research(state: ResearchState) -> ResearchState:
        ...
        for topic in research_topics:
            if "historical" in topic.topic.lower() or "period" in topic.topic.lower():
                # Historical research
                period = topic.topic.replace("Historical Period: ", "").replace(
                    "History of ", ""
                )
                aspects = [topic.description]
                research_item = historical_research.research_historical_period(
                    period, aspects
                )
            elif "technical" in topic.topic.lower() or "domain" in topic.topic.lower():
                # Technical domain research
                domain = topic.topic.replace("Technical Domain: ", "").replace(
                    "Technology of ", ""
                )
                specific_topics = [topic.description]
                research_item = technical_domain.research_domain(
                    domain, specific_topics
                )
            elif "cultural" in topic.topic.lower() or "culture" in topic.topic.lower():
                # Cultural research
                culture = topic.topic.replace("Cultural Elements: ", "").replace(
                    "Culture of ", ""
                )
                elements = [topic.description]
                research_item = cultural_authenticity.research_cultural_elements(
                    culture, elements
                )
            else:
    ...
    def design_structure(state: ResearchState) -> ResearchState:
        ...
        story_structure = structure_specialist.design_structure(
            novel_state, selected_structure
        )
    ...
    def develop_plot_points(state: ResearchState) -> ResearchState:
        ...
        return {**state, "plot_points": plot_points, "novel_state": novel_state}
    ...
    def create_setting_bible(state: ResearchState) -> ResearchState:
        ...
        setting_bible = world_building.create_setting_bible(
            novel_state, world_building_result
        )
        ...
        novel_state.settings = {
            "primary_locations": {
                "description": setting_data.get("expanded_locations", "")
            },
            "world_systems": {"description": setting_data.get("systems_in_depth", "")},
            "timeline": {"description": setting_data.get("timeline", "")},
            "social_structure": {
                "description": setting_data.get("social_structure", "")
            },
            "natural_elements": {
                "description": setting_data.get("flora_fauna_resources", "")
            },
        }

        return {**state, "setting_bible": setting_bible, "novel_state": novel_state}

    # 11. Set next phase
    def set_next_phase(state: ResearchState) -> ResearchState:
        novel_state = state["novel_state"]
        novel_state = project_lead.set_project_phase(
            novel_state, ProjectStatus.CHARACTER_DEVELOPMENT
        )
