...
def create_main_graph(config: storybookConfig = None):
    ...
    def run_research_phase(state: storybookerationState) -> storybookerationState:
        ...
        return {
            **state,
            "novel_state": novel_state,
            "current_phase": novel_state.status.value,
            "phase_states": {**state["phase_states"], "research": subgraph_result},
            "phase_outputs": {
                **state["phase_outputs"],
                "research": {
                    "research_compilation": subgraph_result.get(
    ...
    def run_character_development_phase(
        state: storybookerationState,
    ) -> storybookerationState:
    ...
    def run_drafting_phase(state: storybookerationState) -> storybookerationState:
        ...
        return {
            **state,
            "novel_state": novel_state,
            "current_phase": novel_state.status.value,
            "phase_states": {**state["phase_states"], "drafting": subgraph_result},
            "phase_outputs": {
                **state["phase_outputs"],
                "drafting": {
    ...
    def run_revision_phase(state: storybookerationState) -> storybookerationState:
        ...
        return {
            **state,
            "novel_state": novel_state,
            "current_phase": novel_state.status.value,
            "phase_states": {**state["phase_states"], "revision": subgraph_result},
            "phase_outputs": {
                **state["phase_outputs"],
                "revision": {
                    "structural_analysis": subgraph_result.get(
                        "structural_analysis", {}
                    ),
                    "revision_priorities": subgraph_result.get(
                        "revision_priorities", {}
                    ),
    ...
    def run_reader_optimization_phase(
        state: storybookerationState,
    ) -> storybookerationState:
            ...
            "phase_outputs": {
                **state["phase_outputs"],
                "reader_optimization": {
                    ...
                    "readability_metrics": subgraph_result.get(
                        "readability_metrics", {}
                    ),
                },
            },
    ...
    def run_publication_phase(state: storybookerationState) -> storybookerationState:
        ...
        return {
            **state,
            "novel_state": novel_state,
            "current_phase": novel_state.status.value,
            "phase_states": {**state["phase_states"], "publication": subgraph_result},
            "phase_outputs": {
                **state["phase_outputs"],
                "publication": {
                    ...
                    "positioning_statement": subgraph_result.get(
    ...
    workflow.add_conditional_edges("initialization_phase", route_by_phase)
    workflow.add_conditional_edges("research_phase", route_by_phase)
    workflow.add_conditional_edges("character_development_phase", route_by_phase)
    workflow.add_conditional_edges("drafting_phase", route_by_phase)
    workflow.add_conditional_edges("revision_phase", route_by_phase)
    workflow.add_conditional_edges("reader_optimization_phase", route_by_phase)
    workflow.add_conditional_edges("publication_phase", route_by_phase)
