...
class QualityMetrics:
    ...
    def calculate_thematic_coherence(themes: List[str], text: str) -> float:
        ...
        if word_count > 0:
            normalized_score = term_occurrences / (word_count * 0.01)  # 1 occurrence per 100 words = 1.0
            return min(normalized_score, 1.0)  # Cap at 1.0
        
        return 0.0
    
    @staticmethod
    def calculate_character_consistency(character: Dict[str, Any], dialogue: str) -> float:
        """
        ...
        if "dialogue_patterns" in character:
            ...
            if total_patterns > 0:
                ...
                consistency_score = 0.3 + (0.7 * pattern_score)  # 30% baseline + 70% pattern matching
    ...
    def calculate_narrative_engagement(text: str) -> Dict[str, float]:
        ...
        if total_emotion_words > 0:
            ...
            emotion_distribution = [count/total_emotion_words for count in emotion_counts.values()]
            emotion_variety = min(1.0, 1 - abs(emotion_distribution[0] - 0.33) - 
                                abs(emotion_distribution[1] - 0.33) - abs(emotion_distribution[2] - 0.33))
    ...
    def calculate_readability(text: str) -> Dict[str, float]:
        ...
        flesch_kincaid = 0.39 * (total_words / total_sentences) + 11.8 * (total_syllables / total_words) - 15.59
