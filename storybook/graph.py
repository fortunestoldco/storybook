...
def conduct_market_research(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["message"] = "Completed market research on publishing trends and target audience."
...
def initialize_graph(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["message"] = f"Research and analysis complete. Starting character development for {state['title']}."
...
def develop_characters(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["message"] = f"Enhanced {len(new_state['characters'])} characters based on target audience research."
    new_state["stage_progress"] = {**state.get("stage_progress", {}), "character_development": 1.0}
...
def enhance_dialogue(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["stage_progress"] = {**state.get("stage_progress", {}), "dialogue_enhancement": 1.0}
...
def build_world(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["message"] = f"Enhanced {len(new_state['settings'])} settings based on research."
    new_state["stage_progress"] = {**state.get("stage_progress", {}), "world_building": 1.0}
...
def integrate_subplots(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["stage_progress"] = {**state.get("stage_progress", {}), "subplot_integration": 1.0}
...
def evaluate_story_arcs(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["message"] = result.get("message", "Story arc analysis and refinement completed.")
    new_state["stage_progress"] = {**state.get("stage_progress", {}), "story_arc_evaluation": 1.0}
    ...
    new_state["analysis_results"] = {**new_state.get("analysis_results", {}), **progress_analysis}
...
def check_continuity(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["stage_progress"] = {**state.get("stage_progress", {}), "continuity_check": 1.0}
...
def polish_language(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["stage_progress"] = {**state.get("stage_progress", {}), "language_polishing": 1.0}
...
def review_quality(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_state["stage_progress"] = {**state.get("stage_progress", {}), "quality_review": 1.0}
    ...
    new_state["analysis_results"] = {**new_state.get("analysis_results", {}), **final_analysis}
...
def finalize(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "improvement_metrics": {
            ...
            "language_improvements": state.get("style_analysis", {}).get("improvements_count", 0)
        }
    }
...
def check_analysis_progress(state: Dict[str, Any]) -> Union[Literal["initialize"], Literal["analysis"]]:
    """Check if the analysis is complete and comprehensive enough."""
    analysis_results = state.get("analysis_results", {})
    
    # Check if we have comprehensive analysis data
    required_analysis_types = ["sentiment", "readability", "content_structure", "genre_match"]
...
# Build the graph
def build_novel_transformation_graph() -> StateGraph:
    ...
    workflow.add_conditional_edges(
        ...
        {
            "quality_review": "quality_review",  # Retry if needed
            "finalize": "finalize"
        }
    )
...
novel_transformation_graph = build_novel_transformation_graph().compile()
