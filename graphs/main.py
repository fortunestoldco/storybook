...
def save_publication_package(state: Dict[str, Any], output_dir: str) -> None:
    ...
    package_path = os.path.join(publication_dir, f"{project_name}_publication_package_{timestamp}.json")
    
    with open(package_path, "w") as f:
        json.dump(state["publication_package"], f, indent=2, default=str)
    
    # Also create a readable version
    readable_path = os.path.join(publication_dir, f"{project_name}_publication_package_{timestamp}.txt")
