...
from ...agents.writing import WritingSupervisorAgent, ChapterWriterAgent, ContinuityManagerAgent, DescriptionSpecialistAgent
from ...agents.story_architecture import PlotDevelopmentAgent
...
def create_drafting_graph(config: NovelGenConfig):
    ...
    def draft_chapters(state: DraftingState) -> DraftingState:
        ...
        if "sections" in writing_plan and "stylistic_guidelines" in writing_plan["sections"]:
    ...
    def check_continuity(state: DraftingState) -> DraftingState:
        ...
        for chapter_num, chapter in drafted_chapters.items():
            ...
            novel_state.chapters[chapter_num].quality_metrics["continuity"] = continuity_check["consistency_score"]
    ...
    def enhance_descriptions(state: DraftingState) -> DraftingState:
        ...
        for chapter_num, chapter in drafted_chapters.items():
            ...
            quality_analysis = description_specialist.analyze_description_quality(enhanced_chapter.content)
            novel_state.chapters[chapter_num].quality_metrics["description_quality"] = quality_analysis["overall_score"]
    ...
    def set_next_phase(state: DraftingState) -> DraftingState:
        novel_state = state["novel_state"]
        novel_state = project_lead.set_project_phase(novel_state, ProjectStatus.REVISING)
