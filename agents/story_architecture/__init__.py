...
class StoryStructure(BaseModel):
    ...
    act_breakdown: Dict[str, str] = Field(description="Breakdown of acts/sections with descriptions")
    major_plot_points: List[Dict[str, str]] = Field(description="Major plot points with descriptions")
    estimated_word_counts: Dict[str, int] = Field(description="Estimated word counts for each section")
...
class ChapterOutline(BaseModel):
    ...
    character_development: Dict[str, str] = Field(description="Character development points")
    plot_advancement: List[str] = Field(description="How the plot advances in this chapter")
    themes_explored: List[str] = Field(description="Themes explored in this chapter")
    estimated_word_count: int = Field(description="Estimated word count for this chapter")
...
""",
    ...
    def design_structure(self, state: NovelState, structure_type: str) -> StoryStructure:
...
""",
            ...
            input_variables=["structure_type", "project_name", "genre", "premise", "themes", "target_word_count"]
        )
...
class PlotDevelopmentAgent:
    ...
    def develop_plot_points(self, state: NovelState, structure: StoryStructure) -> List[PlotPoint]:
...
TENSION:
""",
            input_variables=["project_name", "genre", "premise", "structure_type", "act_breakdown", "major_plot_points"]
        )
        ...
        for line in response.content.split("\n"):
            ...
            if ":" in line and line.split(":")[0] in ["TITLE", "DESCRIPTION", "CHARACTERS", "STORY POSITION", "TENSION"]:
                current_field = line.split(":")[0]
                value = line.split(":", 1)[1].strip()
                current_point[current_field] = value
                
                # If we've completed a plot point, add it to the list
                if all(field in current_point for field in ["TITLE", "DESCRIPTION", "CHARACTERS", "TENSION"]):
                    try:
                        tension = float(current_point.get("TENSION", "5").replace("/10", "").strip())
                    except:
                        tension = 5.0
    ...
    def create_chapter_outlines(self, state: NovelState, plot_points: List[PlotPoint]) -> List[ChapterOutline]:
...
""",
            partial_variables={"format_instructions": parser.get_format_instructions()},
            input_variables=["project_name", "genre", "target_word_count", "characters", "plot_points", "chapter_count"]
        )
        ...
        try:
            ...
            return [
                ChapterOutline(
                    ...
                    character_development={"Protagonist": "Initial character state established"},
...
""",
    ...
    def create_setting_bible(self, state: NovelState, world_building: Dict[str, str]) -> Dict[str, Any]:
...
""",
            input_variables=["project_name", "genre", "locations", "rules_systems", "sensory_landscape", "cultural_elements"]
        )
        ...
        return {
            "setting_bible": response.content,
            "sections": sections
        }
