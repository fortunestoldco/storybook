...
""",
            input_variables=["project_name", "genre", "premise", "target_word_count", "character_summary", "plot_summary"]
        )
        
        # Create character summary for the prompt
        character_summary = ""
        for name, character in state.characters.items():
            character_summary += f"{name}: {character.role} - {character.background[:100]}...\n"
        ...
        for line in response.content.split("\n"):
            if "CHAPTER SEQUENCING" in line or "1. CHAPTER SEQUENCING" in line:
                ...
                continue
            elif "CONSISTENCY REQUIREMENTS" in line or "4. CONSISTENCY REQUIREMENTS" in line:
        ...
        return {
            "writing_plan": response.content,
            "sections": sections
        }
    
    def assess_chapter_quality(self, chapter: Chapter, quality_criteria: Dict[str, Any]) -> Dict[str, Any]:
...
""",
            input_variables=["title", "number", "pov_character", "summary", "content", "quality_criteria"]
        )
...
""",
        ...
        for num, chapter in sorted(state.chapters.items()):
            quality_str = ", ".join([f"{k}: {v:.2f}" for k, v in chapter.quality_metrics.items()]) if chapter.quality_metrics else "Not assessed"
        ...
        for line in response.content.split("\n"):
            if "PRIORITY REVISIONS" in line or "1. PRIORITY REVISIONS" in line:
                ...
                continue
            elif "STRUCTURAL RECOMMENDATIONS" in line or "3. STRUCTURAL RECOMMENDATIONS" in line:
                current_section = "structural_recommendations"
                continue
            elif "THEMATIC REINFORCEMENT" in line or "4. THEMATIC REINFORCEMENT" in line:
...
class ChapterWriterAgent:
    ...
    def write_chapter(self, chapter_outline: Any, state: NovelState, style_guide: Dict[str, Any]) -> Chapter:
        ...
        for name, character in state.characters.items():
            if name == chapter_outline.pov_character:
                ...
                characters_info += f"{name}: {character.role} - {character.background[:100]}...\n"
        
        # Get setting information
        settings_info = ""
        for name, setting in state.settings.items():
            settings_info += f"{name}: {setting.get('description', 'No description')}...\n"
...
""",
            input_variables=["genre", "title", "chapter_number", "pov_character", "setting", "summary", 
                           "opening_hook", "closing_hook", "character_development", "plot_advancement", 
                           "themes_explored", "characters_info", "settings_info", "style_guide", "word_count"]
        )
        
        # Format character development for the prompt
        character_dev = "\n".join([f"- {char}: {dev}" for char, dev in chapter_outline.character_development.items()])
        
        # Format plot advancement for the prompt
        plot_adv = "\n".join([f"- {point}" for point in chapter_outline.plot_advancement])
    ...
    def revise_chapter(self, chapter: Chapter, revision_notes: Dict[str, Any], style_guide: Dict[str, Any]) -> Chapter:
...
""",
            input_variables=["title", "chapter_number", "pov_character", "summary", "content", "revision_notes", "style_guide"]
        )
...
class ContinuityManagerAgent:
    ...
    def check_continuity(self, current_chapter: Chapter, previous_chapters: List[Chapter]) -> Dict[str, Any]:
...
""",
            input_variables=["current_title", "current_number", "current_content", "previous_chapters_summary"]
        )
        
        # Create summary of previous chapters
        previous_chapters_summary = ""
        for chapter in previous_chapters[-3:]:  # Limit to the last 3 chapters for token constraints
        ...
        response = self.llm.invoke(prompt.format(
            ...
            current_content=current_chapter.content[:2000] + "..." if len(current_chapter.content) > 2000 else current_chapter.content,
        ...
        for line in response.content.split("\n"):
            if any(line.startswith(prefix) for prefix in ["1.", "2.", "3.", "4.", "5.", "Issue", "Inconsistency"]):
                if current_issue:
...
""",
        ...
        return {
            "elements": sections,
            "tracking_report": response.content
        }
...
class DescriptionSpecialistAgent:
    ...
    def enhance_descriptions(self, chapter: Chapter, settings: Dict[str, Any]) -> Chapter:
...
""",
        ...
        for name, setting in settings.items():
            settings_text += f"{name}: {setting.get('description', 'No description available')}\n"
...
""",
        ...
        # Extract scores from the response
        scores = {}
        aspects = ["sensory balance", "descriptive techniques", "setting immersion", 
                   "character physicality", "emotional atmosphere", "descriptive language"]
