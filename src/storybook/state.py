...
class WritingAgentState(AgentState):
    ...
    collaborating_with: List[str] = Field(default_factory=list)  # Other writers collaborating on joint sections
...
class JointWriterAgentState(WritingAgentState):
    ...
    component_writers: List[str] = Field(default_factory=list)  # IDs of writers contributing to this joint entity
    section_contributions: Dict[str, Dict[str, str]] = Field(default_factory=dict)  # Section ID -> {writer_id: contribution}
...
class GraphState(BaseModel):
    ...
    def add_task(self, task_id: str, agent_id: str, task_type: str, description: str, 
                status: str = "assigned", data: Optional[Dict[str, Any]] = None):
    ...
    def assign_writer_to_sections(self, writer_id: str, section_ids: List[str], joint_llm: bool = False):
        """Assign a writer to specific story sections."""
        # Update the story's writer assignments
        if self.story:
            self.story.assign_writer(writer_id, section_ids, joint_llm)
            
        # Also track in graph state
        ...
        # Update writer's state
        if writer_id in self.writing_team:
            writer_state = self.writing_team[writer_id]
            writer_state.assigned_sections.extend([s for s in section_ids if s not in writer_state.assigned_sections])
    ...
    def create_section_from_template(self, act_index: int, component_index: int, title: Optional[str] = None) -> StorySection:
        ...
        # Use provided title or component name
