...
def project_initiation(state: StoryCreationState) -> Dict:
    ...
    return {
        ...
        "current_step": "Market Research",
        "project_status": "In Research",
        "project_progress": 0,
        "project_goal": "Create a best-selling class novel that ticks all the boxes.",
    }  # added project goal
...
def market_research(state: StoryCreationState) -> Dict:
    ...
    return {
        ...
        "current_step": "Contextual Research",
        "project_progress": 10,
    }  # Set to 10% as example
...
def contextual_research(state: StoryCreationState) -> Dict:
    ...
    return {
        ...
        "current_step": "Consumer Research",
        "project_progress": 20,
    }
...
def consumer_research(state: StoryCreationState) -> Dict:
    ...
    return {
        ...
        "current_step": "Research Supervisor Review",
        "project_progress": 30,
    }
...
def research_supervisor_review(state: StoryCreationState) -> Dict:
    ...
    return {
        ...
        "current_step": "Overall Supervisor Review",
        "project_progress": 40,
    }  # Store compiled research report into the research reports array.
...
def world_building(state: StoryCreationState) -> Dict:
    ...
    return {
        ...
        "current_step": "Character Development",
        "project_progress": 44,
    }
...
def character_development(state: StoryCreationState) -> Dict:
    ...
    return {
        ...
        "current_step": "Writing Team Supervisor Review",
        "project_progress": 46,
    }
...
def writing_team_supervisor_review(state: StoryCreationState) -> Dict:
    ...
    return {
        ...
        "current_step": "Editorial Review (Initial)",
        "project_progress": 48,
    }
...
def editorial_review_initial(state: StoryCreationState) -> Dict:
    ...
    return {
        ...
        "current_step": f"Handle Editorial Grade ({editorial_grade})",
        "project_progress": 50,
    }
...
def handle_editorial_grade(state: StoryCreationState) -> Dict:
    ...
    if grade == "low":
        ...
        next_step = (
            "Writing Team Supervisor Review"  # Rewrite = Writing Team Supervisor Review
        )
    else:
        next_step = (
            "Author Liaison Chat Session"  # Good or amazing =  Author Liason chat
        )
...
def author_liaison_chat_session(state: StoryCreationState) -> Dict:
    ...
    project_progress = 52
    return {
        "current_step": "Story and Dialogue Writers Draft Chapters",
        "project_status": project_status,
        "project_progress": project_progress,
    }
...
def story_and_dialogue_writers_draft_chapters(state: StoryCreationState) -> Dict:
    ...
    chapter_1_number = (
        state.chapters_written + 1
    )  # Chapter 1 #This should be a global incremental change
    ...
    chapter_1_dialogue = llm.invoke(
        prompt_dialogue
    ).content  # Get dialogue writers opinion.
    ...
    chapter_1_draft = (
        f"""Story Writer: {chapter_1_story}\n\nDialogue Writer: {chapter_1_dialogue}"""
    )
    chapter_2_draft = (
        f"""Story Writer: {chapter_2_story}\n\nDialogue Writer: {chapter_2_dialogue}"""
    )

    add_to_story_bible(
        chapter_1_draft,
        {
            "step": "story_and_dialogue_writers_draft_chapters",
            "chapter": chapter_1_number,
        },
    )
    add_to_story_bible(
        chapter_2_draft,
        {
            "step": "story_and_dialogue_writers_draft_chapters",
            "chapter": chapter_2_number,
        },
    )  # Stores the chapter draft to the book bible
    ...
    return {
        ...
        "current_step": "Editorial Team Supervisor Review (Chapters)",
...
def editorial_team_supervisor_review_chapters(state: StoryCreationState) -> Dict:
    ...
    report_editorial_feedback = llm.invoke(
        prompt_editorial_feedback
    ).content  # Chapter editorial feedback.
    editorial_grade = "good"

    editorial_notes_chapter = state.editorial_notes.copy()  # Get the edit notes

    if (
        state.overall_manuscript_draft_number < 3 and state.chapters_written < 10
    ):  # Limit to 3 drafts, and only up to 10 chapter increments.
        ...
        editorial_notes_chapter[chapter_2_number].append(
...
)  # liason between human

# Writing Subgraph
builder.add_node(
    "story_and_dialogue_writers_draft_chapters",
    story_and_dialogue_writers_draft_chapters,
)  # drafts
builder.add_node(
...
)  # give it to editorial team
builder.add_edge(
    "editorial_review_initial", "handle_editorial_grade"
)  # Editorial review grading
...
def run_graph(
    graph: StateGraph, initial_state: Dict[str, Any] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        config = RunnableConfig(
            recursion_limit=100, tags=["storybook_workflow"]  # Prevent infinite loops
        )
...
if __name__ == "__main__":
    ...
    try:
        graph = build_graph()
        result = run_graph(graph, initial_state)
