...
def update_story_bible(state: GraphState) -> GraphState:
    ...
    update_messages = [msg for msg in new_state.messages 
                      if msg["recipient"] == style_guide_id 
...
def create_story_outline(state: GraphState) -> GraphState:
    ...
    writer_id = next(w_id for w_id, w in new_state.writing_team.items()
    ...
    try:
        ...
        if "output" in response:
            ...
            new_state.update_task_status(
                task_id=task_id,
                status="completed",
                result={"outline": response["output"]}
            )
            ...
            review_prompt = REVIEW_TASK_PROMPT.format(
                ...
                context=new_state.user_request.to_prompt_string() if new_state.user_request else "No specific request details provided",
                previous_feedback=""
            )
            ...
            if request_human_feedback:
                ...
                if outline_messages:
                    ...
                    human_review_result = request_human_review(
                        ...
                        content=outline,
                        options=[
                            {"text": "Approve", "description": "Approve the outline and continue to writing phase"},
                            {"text": "Request Revisions", "description": "Request specific revisions to the outline"},
                            {"text": "Reject", "description": "Reject the outline and restart the planning phase"}
                        ],
                        context=f"Supervisor's review:\n\n{response['output']}\n\nStory Structure: {new_state.story_structure.value}",
                        deadline=None
                    )
...
def complete_planning_phase(state: GraphState, review_output: str) -> GraphState:
    ...
    try:
        ...
        if characters_section:
            character_blocks = re.findall(r'### (.+?)(?:\s*):?\s*([\s\S]+?)(?=###|$)', characters_section.group(1))
        ...
        if new_state.story_structure == StoryStructure.THREE_ACT:
            act_pattern = r'## Act (?:I|II|III|1|2|3)(?:\s*):?\s*(.+?)(?:\s*):?\s*([\s\S]+?)(?=## Act|$)'
            ...
            for i, stage in enumerate(journey_stages):
                stage_pattern = f'(?:##|###) {re.escape(stage)}(?:\s*):?\s*([\s\S]+?)(?=(?:##|###)|$)'
        ...
        if not plot_points:
            plot_section = re.search(r'## (?:Plot Points|Plot)(?:\s*):?\s*([\s\S]+?)(?=##|$)', outline)
            if plot_section:
                plot_blocks = re.findall(r'(?:\d+\.\s*|\*\s*|-\s*|###\s*)([\s\S]+?)(?=\d+\.\s*|\*\s*|-\s*|###\s*|$)', plot_section.group(1))
                for i, plot in enumerate(plot_blocks):
                    if plot.strip():
                        plot_title_content = plot.split(':', 1) if ':' in plot else (f"Plot Point {i+1}", plot)
                        ...
                        plot_points.append(PlotPoint(
                            title=title.strip() if isinstance(title, str) else f"Plot Point {i+1}",
                            description=content.strip() if isinstance(content, str) else plot.strip(),
                            sequence=i+1
                        ))
        
        # Extract settings (simplified)
        settings_section = re.search(r'## (?:Settings|Setting|Locations)(?:\s*):?\s*([\s\S]+?)(?=##|$)', outline)
        settings = []
        if settings_section:
            setting_blocks = re.findall(r'(?:\d+\.\s*|\*\s*|-\s*|###\s*)([\s\S]+?)(?=\d+\.\s*|\*\s*|-\s*|###\s*|$)', settings_section.group(1))
            for i, setting in enumerate(setting_blocks):
                if setting.strip():
                    setting_title_content = setting.split(':', 1) if ':' in setting else (f"Setting {i+1}", setting)
                    ...
                    settings.append(Setting(
                        ...
                        description=content.strip() if isinstance(content, str) else setting.strip()
                    ))
        ...
        themes = []
        if themes_section:
            theme_blocks = re.findall(r'(?:\d+\.\s*|\*\s*|-\s*)([\s\S]+?)(?=\d+\.\s*|\*\s*|-\s*|$)', themes_section.group(1))
        ...
        update_result = update_story(
            ...
            {
                "title": title,
                "outline": story_outline.dict(),
                "state": StoryState.WRITING.value
            }
        )
        
        if update_result.get("status") != "success":
            new_state.add_error("system", f"Failed to update story with outline: {update_result.get('message')}")
        ...
        # Plot section
        plot_content = "\n\n".join([f"**{plot.title}** ({plot.act or 'Main Plot'}):\n{plot.description}" 
        ...
        structure_content = f"# {new_state.story_structure.value.replace('_', ' ').title()} Structure\n\n"
        ...
        if story_sections:
            ...
            update_story(
                new_state.story_id,
                {
                    "sections": [section.dict() for section in story_sections]
                }
            )
    ...
    if new_state.writer_assignments:
        ...
        for assignment in new_state.writer_assignments:
            ...
            if new_state.story and new_state.story.sections:
                section = next((s for s in new_state.story.sections if s.id == first_section_id), None)
            ...
            if not section:
                ...
                bible_sections = ""
                from agents.tools import get_bible_sections
                bible_result = get_bible_sections(new_state.story_id, BibleSectionType.PLOT_ELEMENTS.value)
                
                if bible_result.get("status") != "error" and "sections" in bible_result:
                    plot_sections = bible_result["sections"].get(BibleSectionType.PLOT_ELEMENTS.value, [])
...
def write_story_section(state: GraphState) -> GraphState:
    ...
    section_messages = [msg for msg in new_state.messages 
                       if msg["recipient"] == writer_id 
                       and (msg.get("metadata", {}).get("task_type") == "write_section" or
                            msg.get("metadata", {}).get("task_type") == "write_draft")
                       and (msg.get("metadata", {}).get("section_id") == section_id or
                            msg.get("metadata", {}).get("section") == section_id)]
        ...
        if "output" in response:
            ...
            if isinstance(writer.assigned_sections, list):
                ...
                for assigned_section in writer.assigned_sections:
                    if assigned_section != section_id and assigned_section not in writer.completed_sections:
                        ...
                        next_section = None
                        if new_state.story and new_state.story.sections:
                            next_section = next((s for s in new_state.story.sections if s.id == next_section_id), None)
                        ...
                        section_title = next_section.title if next_section else f"Section {next_section_id}"
...
def build_graph():
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: state.operation_mode.value if state.operation_mode else OperationMode.CREATE.value,
        {
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: "awaiting_human_input" if state.awaiting_human_input else "execute_edit_continuation",
        {
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: "start_publishing_phase" if state.current_state == StoryState.READY_FOR_PUBLISHING else "execute_edit_continuation",
        {
    ...
    workflow.add_conditional_edges(
        "conduct_briefing",
        lambda state: "awaiting_human_input" if state.awaiting_human_input else "start_research_phase",
        {
    ...
    workflow.add_conditional_edges(
        "review_research",
        lambda state: "conduct_research" if any(r.status == "revising_research" for r in state.research_team.values()) 
                 else "update_story_bible",
        {
            "conduct_research": "conduct_research",
            "update_story_bible": "update_story_bible"
        }
    )
    ...
    workflow.add_conditional_edges(
        "review_outline",
        lambda state: "awaiting_human_input" if state.awaiting_human_input else 
                 "create_story_outline" if any(w.status == "revising_outline" for w in state.writing_team.values()) 
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: "create_story_outline" if any(w.status == "human_outline_revision" for w in state.writing_team.values())
                 else "start_writing_phase" if state.current_state == StoryState.WRITING
                 else "start_planning_phase",  # For full restarts
        {
            "create_story_outline": "create_story_outline",
            "start_writing_phase": "start_writing_phase",
    ...
    workflow.add_conditional_edges(
        "write_story_section",
        lambda state: "write_story_section" if any(w.status not in ["idle", "error", "completed"] for w in list(state.writing_team.values()) + list(state.joint_writers.values()))
    ...
    workflow.add_conditional_edges(
        "review_final_draft",
        lambda state: "awaiting_human_input" if state.awaiting_human_input 
                 else "edit_story" if any(e.status == "revising" for e in state.editing_team.values())
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: "edit_story" if any(e.status == "human_revisions" for e in state.editing_team.values())
                 else "start_publishing_phase",
        {
            "edit_story": "edit_story",
            "start_publishing_phase": "start_publishing_phase"
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: "awaiting_human_input" if state.awaiting_human_input 
                 else "prepare_for_publishing" if any(p.status == "revising_publishing" for p in state.publishing_team.values())
                 else "publish_story",
        {
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: "prepare_for_publishing" if any(p.status in ["human_publishing_changes", "major_publishing_revision"] 
    ...
    workflow.add_edge("execute_publishing", "execute_publishing", 
...
```json
{
  "graphs": {
    "storybook_graph": {
      "description": "Multi-agent system for collaborative story creation with research, writing, and publishing teams",
      "entrypoint": "agents.graph:graph"
    }
  }
}
