...
def parse_feedback(feedback_text: str) -> Dict[str, Any]:
    ...
    result = {"strengths": [], "weaknesses": [], "suggestions": [], "rating": None}
    ...
    strengths_match = re.search(
        r"strengths?[:\s]+(.*?)(?=weaknesses?|improvements?|suggestions?|$)",
        feedback_text,
    ...
    weaknesses_match = re.search(
        r"(?:weaknesses?|improvements?)[:\s]+(.*?)(?=suggestions?|strengths?|$)",
        feedback_text,
        re.IGNORECASE | re.DOTALL,
    )
    if weaknesses_match:
    ...
    suggestions_match = re.search(
        r"suggestions?[:\s]+(.*?)(?=strengths?|weaknesses?|improvements?|$)",
        feedback_text,
        re.IGNORECASE | re.DOTALL,
    )
...
def format_agent_response(
    ...
    return {"content": content, "metadata": metadata, "timestamp": current_timestamp()}
...
def prepare_human_review_prompt(
    review_type: str, content: str, options: List[Dict[str, Any]], context: Optional[str] = None
) -> str:
...
def create_model_instance(model_type: str = None, use_local: bool = None):
    ...
    if use_local:
        ...
        return ChatOllama(
            model=model_name, temperature=temperature, base_url=config.OLLAMA_BASE_URL
        )
    else:
        # Use OpenAI models
        model_name_map = {
