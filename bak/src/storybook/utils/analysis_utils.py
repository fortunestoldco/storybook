...
from storybook.text_utils import (
    ...
    extract_entities,
    detect_dialogue,
    detect_narrative_perspective,
    detect_reading_level,
    detect_style_elements,
...
def analyze_thematic_coherence(text: str, themes: List[str]) -> Dict[str, Any]:
    ...
    for theme in themes:
        ...
        if theme.lower() == "love":
            theme_terms.update(
                ["romance", "affection", "passion", "heart", "emotional", "relationship"]
            )
        elif theme.lower() == "betrayal":
            theme_terms.update(
                ["treachery", "deception", "dishonesty", "treason", "disloyal", "backstab"]
            )
        elif theme.lower() == "redemption":
            theme_terms.update(
                ["forgiveness", "atonement", "salvation", "reconciliation", "amends"]
            )
        elif theme.lower() == "justice":
            theme_terms.update(
                ["fairness", "equality", "rights", "law", "moral", "ethics", "punishment"]
            )
        elif theme.lower() == "power":
            theme_terms.update(
                ["control", "authority", "dominance", "strength", "influence", "force"]
            )
    ...
    if themes:
        ...
        variance = sum(
            (presence - avg_presence) ** 2 for presence in theme_presence.values()
        ) / len(themes)
        ...
        keyword_alignment = sum(1 for keyword in keywords if keyword in theme_terms_all) / max(
            1, len(keywords)
        )
        topic_alignment = sum(1 for topic in topics if topic in theme_terms_all) / max(
            1, len(topics)
        )
...
def analyze_character_consistency(text: str, characters: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for character in characters:
        ...
        mentions = sum(
            1 for sentence in sentences if re.search(name_pattern, sentence, re.IGNORECASE)
        )
        ...
        dialogue_pattern = (
            r"(?:"
            + re.escape(name)
            + r'\s*(?:said|replied|asked|exclaimed|whispered|shouted|answered),?\s*"|"\s*(?:said|replied|asked|exclaimed|whispered|shouted|answered)\s*'
            + re.escape(name)
            + r")"
        )
        character_dialogues = []
    ...
    for character in characters:
        ...
        for trait in traits:
            ...
            if dialogue_length + action_length > 0:
                normalized_presence = (
                    (trait_count + trait_action_count) / (dialogue_length + action_length) * 100
                )
        ...
        if trait_presence:
            ...
            if len(trait_presence) > 1:
                variance = sum((presence - avg_presence) ** 2 for presence in trait_presence) / len(
                    trait_presence
                )
            else:
                variance = 0
    ...
    return {
        ...
        "dialogue_samples": {
            name: dialogues[:3] for name, dialogues in character_dialogue.items() if dialogues
        },
        "action_samples": {
            name: actions[:3] for name, actions in character_actions.items() if actions
        },
    }
...
def analyze_narrative_engagement(text: str) -> Dict[str, Any]:
    ...
    for section in sections:
        ...
        if sentence_lengths:
            ...
            pacing_metrics.append({"avg_sentence_length": avg_length, "variance": variance})
    ...
    for section in sections:
        ...
        emotional_arc.append({"sentiment": sentiment["sentiment"], "score": sentiment["score"]})
    ...
    if sentiment_scores:
        ...
        emotional_arc_completeness = min(
            1.0,
            (0.5 * emotional_range + 0.5 * (sentiment_shifts / max(1, len(sentiment_scores) - 1))),
        )
    else:
        emotional_range = 0.0
        sentiment_shifts = 0
    ...
    if len(sentiment_scores) >= 3:
        ...
        beginning_sentiment = sum(sentiment_scores[: len(sentiment_scores) // 3]) / (
            len(sentiment_scores) // 3
        )
        middle_sentiment = sum(
            sentiment_scores[len(sentiment_scores) // 3 : 2 * len(sentiment_scores) // 3]
        ) / (len(sentiment_scores) // 3)
        end_sentiment = sum(sentiment_scores[2 * len(sentiment_scores) // 3 :]) / (
            len(sentiment_scores) - 2 * len(sentiment_scores) // 3
        )
        ...
        has_resolution = (
            end_sentiment > middle_sentiment or abs(end_sentiment - beginning_sentiment) < 0.2
        )
...
def calculate_quality_improvement(original_text: str, revised_text: str) -> Dict[str, Any]:
    ...
    word_count_change_percent = (
        (word_count_change / original_stats["word_count"]) * 100
        if original_stats["word_count"] > 0
        else 0
    )

    lexical_diversity_change = (
        revised_stats["lexical_diversity"] - original_stats["lexical_diversity"]
    )
    lexical_diversity_change_percent = (
        (lexical_diversity_change / original_stats["lexical_diversity"]) * 100
        if original_stats["lexical_diversity"] > 0
        else 0
    )
    ...
    dialogue_percentage_change = (
        revised_dialogue["dialogue_percentage"] - original_dialogue["dialogue_percentage"]
    )

    # Calculate overall improvement score
    # This is a weighted combination of various metrics
    technical_improvement = min(
        1.0,
        max(
            0.0,
            (
                ...
                + 0.5
                * (
                    abs(reading_ease_change) / 20
                )  # Normalize to 0-1, any change (up or down) could be improvement
            ),
        ),
    )

    stylistic_improvement = min(
        1.0,
        max(
            0.0,
            (
                0.7
                * (style_complexity_change + 0.5)  # Normalize to 0-1, assuming -0.5 to +0.5 range
                + 0.3 * (abs(dialogue_percentage_change) / 10)  # Normalize to 0-1
            ),
        ),
    )

    overall_improvement = 0.6 * technical_improvement + 0.4 * stylistic_improvement
...
def evaluate_story_quality(
    text: str, themes: List[str] = None, characters: List[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
