...
async def execute_bible_update(
    ...
    try:
        ...
        if "output" in response:
            ...
            return format_agent_response(
                content="Error: No output from style guide editor agent", metadata={"error": True}
            )
    except Exception as e:
        return format_agent_response(
            content=f"Error updating bible: {str(e)}", metadata={"error": True}
        )


async def analyze_imported_content(
    agent_id: str, story_id: str, imported_content: str, user_requirements: str = ""
) -> Dict[str, Any]:
    ...
    try:
        ...
        response = await agent.ainvoke(
            {"input": analysis_prompt, "agent_id": agent_id, "story_id": story_id}
        )
        ...
        return format_agent_response(
            content=f"Error analyzing imported content: {str(e)}", metadata={"error": True}
        )
