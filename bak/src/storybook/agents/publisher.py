...
def create_publishing_task(
    ...
    return PUBLISHING_TASK_PROMPT.format(
        ...
        or "Website, blog, or other digital platforms as appropriate.",
        seo_goals=seo_goals
        or "Maximize discoverability through effective keywords, meta description, and categorization.",
    )
...
async def execute_publishing_task(
    ...
    try:
        ...
        if "output" in response:
            ...
            return format_agent_response(
                content="Error: No output from publisher agent", metadata={"error": True}
            )
    except Exception as e:
        return format_agent_response(
            content=f"Error during publishing preparation: {str(e)}", metadata={"error": True}
        )
...
async def execute_publish_action(
    ...
    try:
        ...
        response = await agent.ainvoke(
            {"input": prompt, "agent_id": agent_id, "story_id": story_id, "platform": platform}
        )
        ...
        return format_agent_response(
            content=f"Error during publishing execution: {str(e)}", metadata={"error": True}
        )
