...
async def execute_outline_review(
    ...
    try:
        ...
        structure_reference = (
            f"adherence to the {story_structure} structure"
            if story_structure
            else "structural coherence"
        )
        ...
        return format_agent_response(
            content=f"Error during outline review: {str(e)}", metadata={"error": True}
        )
...
async def execute_draft_review(
    ...
    try:
        ...
        task_prompt = create_review_task(
            ...
            content_summary=(
                draft_content[:5000] + "..." if len(draft_content) > 5000 else draft_content
            ),
            review_criteria=review_criteria,
            story_structure=story_structure,
            context=context,
        ...
        return format_agent_response(
            content=f"Error during draft review: {str(e)}", metadata={"error": True}
        )
