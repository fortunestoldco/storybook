...
state_graph.add_transition(
    ...
    "conduct_briefing",
    condition=lambda state: state.operation_mode == OperationMode.CREATE,
)
state_graph.add_transition("conduct_briefing", "process_human_briefing_input")
state_graph.add_transition(
    "process_human_briefing_input",
    "plan_edit_continuation",
    condition=lambda state: state.current_state == StoryState.PLANNING,
)
state_graph.add_transition(
    "process_user_request",
    "analyze_imported_content",
    condition=lambda state: state.operation_mode == OperationMode.IMPORT,
)
...
state_graph.add_transition(
    "process_user_request",
    "plan_edit_continuation",
    condition=lambda state: state.operation_mode in [OperationMode.EDIT, OperationMode.CONTINUE],
)
...
state_graph.add_transition(
    "review_edit_continuation", "process_human_edit_approval", condition=lambda state: True
)
state_graph.add_transition(
    "process_human_edit_approval",
    "start_publishing_phase",
    condition=lambda state: state.current_state == StoryState.READY_FOR_PUBLISHING,
)
